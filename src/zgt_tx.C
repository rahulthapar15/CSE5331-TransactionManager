...
void *readtx(void *arg){
  ...
  /* Acquire the shared lock and find the object that current transaction requires.
   * if ABORTED : no operation performed
   * if ACTIVE  :  set_lock -> look for object in hash table
   * if WAIT    : release Tx manager
   * if END     : abort and release Tx manager
   * set_lock returns TRUE -> S lock recieved -> perform Read operation
   *
  */
...
void *writetx(void *arg){
  ...
  /* Acquire the exclusive lock and find the object that current transaction requires.
   * if ABORTED : no operation performed
   * if ACTIVE  : set_lock -> look for object in hash table
   * if WAIT    : release Tx manager
   * if END     : abort and release Tx manager
...
{ 
  /* change the status of current transaction to ABORT
   * do_commit_abort -> free locks held by transaction
  */
  struct param *node = (struct param*)arg;
  start_operation(node->tid, node->count); 
  zgt_p(0);
...
{
  /* change the status of current transaction to COMMIT
   * do_commit_abort -> free locks held by transaction
  */
  struct param *node = (struct param*)arg;
  start_operation(node->tid, node->count); 
...
void *do_commit_abort(long t, char status){
	...
	else fprintf(logfile, "T%d\tCommmitTx \t",t);

	fflush(logfile);
	zgt_tx *txptr = get_tx(t);
	if(txptr == NULL)
  {
		fprintf(logfile, "\n Transaction does not exsist : (ABORT/COMMIT)\n");
		fflush(logfile);
		fflush(stdout);
	}
	else{
    ...
    txptr->status = status;
    txptr->free_locks();
    int releaseTransactions = txptr->semno;
    txptr->remove_tx();
    int i, num_transWaiting;
    if (releaseTransactions != -1) {
      num_transWaiting = zgt_nwait(releaseTransactions);
      for(i=1; i<=num_transWaiting; i++)
        zgt_v(releaseTransactions);//does N zgt_v() for N zgt_p(): wake up
      }
	}
}
...
int zgt_tx::set_lock(long tid1, long sgno1, long obno1, int count, char lockmode1){
  ...
  /*
     CHECK if there is a lock on the requested transaction
     1. If lock DOES NOT EXISTS: acquire the lock and continue
     2. If lock DOES EXISTS    :
            Condition 1: 
              If the owner of transaction is the one requesting it,
                => YES: continue
                => NO : acquire S lock (if current hold is also S lock)
  */
...
void zgt_tx::perform_readWrite(long tid,long obno, char lockmode){
  ...
  /*
  * lockmode == 'X': Increment objarray[] by 1
  * lockmode == 'S': Decrement objarray[] by 1
  */
		int val = ZGT_Sh->objarray[obno]->value;

    open_logfile_for_append();
		if (lockmode == 'X') {
			ZGT_Sh->objarray[obno]->value = val + 1;
